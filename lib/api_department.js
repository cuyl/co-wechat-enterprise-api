 ...
 * Examples:
 * ```
 * const result = await api.createDepartment(name, opts);
 * ```
 * Opts:
...
exports.createDepartment = async function(name, opts) {
  ...
  const url = this.prefix + 'department/create?access_token=' + accessToken;
  const data = { name };
 ...
 * Examples:
 * ```
 * const opts = {name: 'new name', parentid: 1, order: 5};
 * const result = await api.updateDepartment(id, opts);
 * ```
 *
 * Opts:
 * - `name`, 新的部门名字。可选
...
exports.updateDepartment = async function(id, opts) {
  ...
  const { accessToken }  = await this.ensureAccessToken();
  const url = this.prefix + 'department/update?access_token=' + accessToken;
 ...
 * const result = await api.deleteDepartment([id1, id2]);
 * ```
 *
 * Result:
 * ```
...
exports.deleteDepartment = async function(id) {
  ...
  const { accessToken }  = await this.ensureAccessToken();
  let url = this.prefix + 'department/delete?access_token=' + accessToken;
 ...
 * ```
 */
exports.getDepartments = async function(callback) {
  // https://qyapi.weixin.qq.com/cgi-bin/department/list?access_token=ACCESS_TOKEN
  const { accessToken }  = await this.ensureAccessToken();
  const url = this.prefix + 'department/list?access_token=' + accessToken;
  const opts = {dataType: 'json'};
  return this.request(url, opts);
};
