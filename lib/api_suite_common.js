 ...
 * ```
 * const SuiteAPI = require('co-wechat-enterprise-api').SuiteAPI;
 * const api = new SuiteAPI('suite_id', 'suite_secret');
 * ```
 * 以上即可满足单进程使用。
 ...
 *
 * ```
 * const api = new SuiteAPI('suite_id', 'suite_secret', function* () {
 *   // 传入一个获取全局token的方法
 ...
 */
const SuiteAPI = function (suite_id, suite_secret, getToken, saveToken) {
...
SuiteAPI.prototype.getAccessToken = function* () {
  const url = this.prefix + 'gettoken?suite_id=' + this.suite_id + '&suite_secret=' + this.suite_secret;
...
SuiteAPI.mixin = function (obj) {
  for (const key in obj) {
    ...
    SuiteAPI.prototype[key] = (obj[key] instanceof Function) ? wrap(obj[key]) : obj[key];
  }
};
