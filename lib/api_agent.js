 ...
 */
exports.getAgent = async function (agentid) {
  const { accessToken } = await this.ensureAccessToken();
  const url = this.prefix + 'agent/get?access_token=' + accessToken + '&agentid=' + agentid;
  const opts = {
    dataType: 'json'
  };
 ...
 * Examples:
 * ```
 * const result = await api.setAgent(opts);
 * ```
 * Opts:
 * ```
 * {
...
exports.setAgent = async function(opts) {
  const { accessToken } = await this.ensureAccessToken();
  const url = this.prefix + 'agent/set?access_token=' + accessToken;
 ...
 * Examples:
 * ```
 * const result = await api.listAgent();
 * ```
 *
 * ```
 *
...
exports.listAgent = async function () {
  const { accessToken } = await this.ensureAccessToken();
  const url = this.prefix + 'agent/list?access_token=' + accessToken;
  const opts = {
    dataType: 'json'
  };
  return this.request(url, opts);
